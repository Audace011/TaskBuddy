  ...
  <Transition name="modal">
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" @click.self="handleCancel">
      ...
      <div class="relative w-full max-w-xl overflow-hidden rounded-custom border border-border-subtle dark:border-slate-800
        ...
        <!-- Header -->
        <div class="mb-8">
          <h2 class="text-app-title text-text-primary dark:text-text-light">Create New Task</h2>
          <p class="mt-1 text-sm text-text-secondary dark:text-slate-400">Add a new task to your list.</p>
        </div>

        <form @submit.prevent="handleCreate" class="space-y-6">
          <!-- Title -->
          <div>
            <label class="mb-2 block text-metadata font-semibold uppercase tracking-wider text-text-secondary">Title</label>
            <input
              v-model="title"
              type="text"
              placeholder="e.g., Design the new home module"
          ...
          <!-- Description -->
          <div>
            <label class="mb-2 block text-metadata font-semibold uppercase tracking-wider text-text-secondary">Description</label>
            <textarea
              v-model="description"
              rows="4"
              placeholder="Any specific notes or sub-tasks for this?"
              class="w-full rounded-custom border border-border-subtle bg-bg-main dark:bg-slate-800
                     px-4 py-3 text-sm font-medium text-text-primary dark:text-text-light
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Priority -->
            <div>
              <label class="mb-2 block text-metadata font-semibold uppercase tracking-wider text-text-secondary">Priority</label>
              <div class="relative">
                <select
                  v-model="priority"
            ...
            <!-- Due Date -->
            <div>
              <label class="mb-2 block text-metadata font-semibold uppercase tracking-wider text-text-secondary">Due Date</label>
              <input
                v-model="dueDate"
                type="date"
                @change="validateDate"
