  ...
  <div class="flex min-h-screen bg-bg-main dark:bg-dark-bg transition-colors duration-200">
    ...
    <main class="ml-64 flex-1 px-8 py-10 lg:px-12">
      ...
      <div class="mb-8 flex items-baseline justify-between transition-all">
        <div>
          <h1 class="text-app-title text-text-primary dark:text-text-light">In Progress</h1>
          <p class="mt-1 text-sm text-text-secondary dark:text-slate-400">
            You are currently working on <span class="text-primary font-bold">{{ filteredTasks.length }}</span> tasks.
          </p>
        </div>
      </div>
      ...
      <div v-else class="flex flex-col items-center justify-center py-24 px-6 text-center">
        ...
        <h3 class="text-task-title text-text-primary dark:text-text-light mb-2">No active tasks</h3>
        <p class="text-body text-text-secondary dark:text-slate-400 max-w-sm">Move tasks to "In Progress" to see them here.</p>
      </div>
